export const TWITCH_AUTH_URL = "https://id.twitch.tv/oauth2/authorize?" + new URLSearchParams({
...
export const fetchFollowedStreams = async (accessToken:string, userId:string) => {
  const url = 'https://api.twitch.tv/helix/streams/followed';
  const queryParams = new URLSearchParams({
    user_id: userId,
  }).toString();
  const response = await fetch(`${url}?${queryParams}`, getTwitchRequestHeaders(accessToken));
  return await response.json();
};
